    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="toss-card p-4 sm:p-6 md:p-8 space-y-3.5 sm:space-y-4">
        ...
        <div className="space-y-3 pt-1 text-xs">
          ...
          <div className="p-3.5 sm:p-4 bg-toss-bg rounded-2xl space-y-1 border-l-4 border-toss-red">
            <span className="font-extrabold text-toss-text text-xs sm:text-sm">청년층 카드 소비 둔화와 러닝 인구의 간극</span>
            <p className="text-toss-subtext leading-relaxed text-[11px] sm:text-xs">
      ...
      <div className="toss-card p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-5 bg-gradient-to-br from-white to-blue-50/40 border border-toss-blue/20">
        ...
        <div className="p-3.5 sm:p-4 rounded-2xl bg-white border border-toss-border space-y-2 text-xs">
          <strong className="text-xs sm:text-sm font-extrabold text-toss-text block">
      ...
      <div className="toss-card p-4 sm:p-6 md:p-7 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-base sm:text-lg font-extrabold text-toss-text flex items-center gap-2">
              <Building2 className="w-5 h-5 text-toss-blue shrink-0" />
              서울 25개 자치구 지표 탐색기
            </h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative flex-1 sm:flex-initial">
              ...
              <input
                ...
                className="bg-toss-bg border-none rounded-2xl pl-9 pr-3 py-2 text-xs font-bold text-toss-text focus:ring-2 focus:ring-toss-blue outline-none w-full sm:w-44"
              />
            </div>
            ...
            <select
              ...
              className="bg-toss-bg border-none rounded-2xl px-3 py-2 text-xs font-bold text-toss-text focus:ring-2 focus:ring-toss-blue outline-none cursor-pointer"
            >
        ...
        {/* 선택된 구 상세 요약 칩 */}
        {currentDistrictData && (
          <div className="p-3.5 sm:p-4 bg-toss-bg rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border border-toss-border/70">
            <div>
              <span className="text-[10px] sm:text-xs font-bold text-toss-blue bg-white px-2 py-0.5 rounded-full shadow-sm">
                현재 선택된 지역
              </span>
              <h3 className="text-lg sm:text-xl font-black text-toss-text mt-1">
            ...
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2 text-xs sm:text-right">
              <div className="bg-white p-2 sm:p-2.5 rounded-xl text-center sm:text-right">
                <span className="text-toss-muted text-[10px] sm:text-[11px] block">유동전환율</span>
                <strong className="text-toss-blue font-black text-xs sm:text-sm">
                  {(currentDistrictData.conversion_rate * 100).toFixed(2)}%
                </strong>
                <span className="text-[9px] sm:text-[10px] text-toss-muted block">({currentDistrictData.conversion_rank}위)</span>
              </div>
              <div className="bg-white p-2 sm:p-2.5 rounded-xl text-center sm:text-right">
                <span className="text-toss-muted text-[10px] sm:text-[11px] block">1인당 소비</span>
                <strong className="text-toss-text font-black text-xs sm:text-sm">
                  {currentDistrictData.per_capita_consumption}건
                </strong>
                <span className="text-[9px] sm:text-[10px] text-toss-muted block">({currentDistrictData.per_capita_rank}위)</span>
              </div>
              <div className="bg-white p-2 sm:p-2.5 rounded-xl text-center sm:text-right">
                <span className="text-toss-muted text-[10px] sm:text-[11px] block">총 유동인구</span>
                <strong className="text-toss-text font-black text-xs sm:text-sm">
        ...
        <div className="pt-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2">
            ...
            <div className="flex items-center p-1 bg-toss-bg rounded-xl text-[11px] sm:text-xs font-bold self-start sm:self-auto overflow-x-auto max-w-full">
              <button
      ...
      <div className="toss-card p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 pb-3 border-b border-toss-border">
          <div>
            <h2 className="text-base sm:text-lg font-extrabold text-toss-text">동작구 업종별 소비 증감률 (2026.01 ~ 2026.06)</h2>
            <p className="text-[11px] sm:text-xs text-toss-subtext mt-0.5">
              BC카드 필수 데이터로 최근 소비가 줄고 있는 골목 취약 업종을 자동 우선 매칭합니다.
            </p>
          </div>
          <div className="flex items-center gap-3 text-xs font-bold">
            <span className="flex items-center gap-1.5 text-toss-red text-[11px] sm:text-xs">
              <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-toss-red"></span> 소비 감소 (지원 우선)
            </span>
            <span className="flex items-center gap-1.5 text-toss-blue text-[11px] sm:text-xs">
              <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-toss-blue"></span> 소비 증가
            </span>
          </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          <div className="lg:col-span-2 h-[220px] sm:h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
