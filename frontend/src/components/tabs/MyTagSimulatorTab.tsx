...
export const MyTagSimulatorTab: React.FC<Props> = ({
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="toss-card p-5 bg-white border border-toss-border space-y-3.5 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="text-xs font-bold text-toss-muted block">현재 조회 중인 지역</span>
              <strong className="text-base font-extrabold text-toss-text block">
                {selectedDistrict} 실제 MyTag 가맹 상권
              </strong>
            </div>
          </div>

          {/* 지역(자치구) 빠른 검색 & 선택 드롭다운 */}
          <div className="flex items-center gap-2">
            <div className="relative">
              <input
                ...
                className="w-28 px-2.5 py-1.5 rounded-xl text-xs bg-toss-bg border border-toss-border text-toss-text outline-none focus:border-toss-blue focus:bg-white transition-all placeholder:text-toss-muted"
              />
            </div>
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none pl-3 pr-8 py-2 rounded-xl text-xs font-extrabold bg-toss-bg border border-toss-border text-toss-text outline-none cursor-pointer hover:bg-gray-200/60 transition-all"
              >
                {filteredDistricts.map(d => (
                  <option key={d} value={d}>{d}</option>
                ))}
              </select>
              <ChevronDown className="w-3.5 h-3.5 text-toss-muted absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" />
            </div>
        ...
        <div className="pt-2 border-t border-toss-border/60 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={`${selectedDistrict} 실제 매장명 또는 대표메뉴 검색 (예: 순천집, 몽탄, 칼국수, 떡볶이, 베이커리)`}
      ...
      <div className="toss-card p-6 md:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h2 className="text-2xl font-extrabold text-toss-text">
        ...
        <div className="bg-toss-bg p-4 rounded-2xl min-w-[170px] text-left sm:text-right border border-toss-border/60">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-6 toss-card p-6 md:p-7 flex flex-col justify-between h-full space-y-5">
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-toss-border">
              <h3 className="text-base font-extrabold text-toss-text flex items-center gap-2">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full p-3.5 pr-9 rounded-2xl bg-toss-bg border border-toss-border text-xs font-extrabold text-toss-text outline-none focus:border-toss-blue focus:bg-white transition-all appearance-none cursor-pointer"
                >
                  {filteredShops.map(s => (
            ...
            {/* 선택된 실제 가맹점 정보 카드 */}
            {selectedShop && (
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-toss-border/80 space-y-2 text-xs">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full p-4 rounded-2xl bg-toss-bg border border-toss-border text-lg font-black text-toss-text outline-none focus:border-toss-blue focus:bg-white transition-all pr-12"
                />
            ...
            {/* 결제 미리보기 요약 */}
            {selectedShop && (
              <div className="p-4 rounded-2xl bg-toss-bg space-y-1.5 text-xs border border-toss-border/40">
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full py-4 rounded-2xl font-black text-sm bg-toss-blue text-white hover:bg-toss-blueHover transition-all flex items-center justify-center gap-2 active:scale-[0.98] shadow-md shadow-blue-500/20 disabled:opacity-50"
            >
              {simulating ? (
        ...
        <div className="lg:col-span-6 toss-card p-6 md:p-7 flex flex-col justify-between h-full space-y-5">
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-toss-border">
              <div>
                <h3 className="text-base font-extrabold text-toss-text">
                  로컬 단골 스탬프 (10회 루프)
                </h3>
              ...
              <span className="text-xs font-black text-toss-blue bg-toss-blueLight px-2.5 py-1 rounded-full">
                {currentStamps} / 10개
              </span>
            </div>

            {/* 10개 스탬프 슬롯 그리드 (2행 5열) */}
                  ...
                  >
                    {isChecked ? (
                      <>
                        ...
                        <span className="text-[10px] font-bold mt-0.5">{stampNum}회</span>
                      </>
                    ) : (
                      <>
                        <span className="text-sm font-black">{stampNum}</span>
            ...
            <div className="space-y-2.5 pt-2">
              ...
              <div className="space-y-2 text-xs">
                <div className="p-3 rounded-2xl bg-toss-bg flex items-center justify-between border border-toss-border/40">
                  <div className="flex items-center gap-2">
                    <span className="text-base">🥉</span>
                    <span className="font-bold text-toss-text">3회 완료 시</span>
                  </div>
                  <span className="font-extrabold text-toss-blue text-right">로컬 카페 아메리카노 1잔 무료</span>
                </div>
                <div className="p-3 rounded-2xl bg-toss-bg flex items-center justify-between border border-toss-border/40">
                  <div className="flex items-center gap-2">
                    <span className="text-base">🥈</span>
                    <span className="font-bold text-toss-text">5회 완료 시</span>
                  </div>
                  <span className="font-extrabold text-toss-blue text-right">페이북 머니 5,000원 추가 캐시백</span>
                </div>
                <div className="p-3 rounded-2xl bg-toss-bg flex items-center justify-between border border-toss-border/40">
                  <div className="flex items-center gap-2">
                    <span className="text-base">🥇</span>
                    <span className="font-bold text-toss-text">10회 완료 시 (VIP)</span>
                  </div>
                  <span className="font-extrabold text-toss-blue text-right">{selectedDistrict} 가맹점 10,000원 바우처</span>
                </div>
              </div>
            </div>
          </div>
